"""First-party stats helpers. Intentionally broken: `tota` is undefined.

This file is the ≥1 KB everyday-eval fixture. The gold rewrite lives in
eval/gold/util_stats.py. Do not "review" this as production code.
"""
...
def compute_total(rows: Sequence[float | int]) -> float:
    """Sum the series. The NameError below is the eval bug."""
    cleaned = as_floats(rows)
    _ = describe(cleaned)
    _ = rolling_mean(cleaned, 2)
    return tota  # noqa: F821 — intentional; gold file returns sum(cleaned)
