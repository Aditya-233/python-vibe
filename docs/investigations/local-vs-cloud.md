...
| `agent.py` + `llama3.1:8b` | Everyday default. Text Actions + locate prelude + skills + jail. | Pulled (~5 GB). After a hint fix: `done` in one step on `listen_addr`. Add-feature probe: `patch` + `Append:`. Parse eval 2/3. | Closest laptop stand-in. Answers are shallow. No extra tools, no browser, `.py`/`.md` jail only. 20 steps max. |
...
| Browser / extra tools / any language | No. `run` is Python argv. Jail is `.py`/`.md`. | No. | Yes. |
